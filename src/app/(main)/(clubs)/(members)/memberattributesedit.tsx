  ...
  const handleSave = async () => {
    if (!isAplhaNumeric(attributeName)) {
      setAlertConfig({
        visible: true,
        title: "Error",
        message: "Enter only alphanumeric with min 2 characters.",
    ...
    <View>
      ...
      {!isLoadingCMA && cmaList?.length == 0 && (
        <ThemedText style={{ width: "80%", alignSelf: "center" }}>
          Define club level attributes which need to be collected from each member. Eg. tshirt size, blood group, id
          number etc.
        </ThemedText>
      )}
      {isLoadingCMA && <LoadingSpinner />}
      {!isLoadingCMA &&
        cmaList?.map((item: { attribute: any; required: number }) => (
            <View>
                <RoundedContainer>
          <TouchableCard
          ...
          >
            <ThemedText>
              {item.attribute} {item.required == 1 ? "*" : ""}
            </ThemedText>
          </TouchableCard></RoundedContainer>
          <Spacer space={4} />
          </View>
        ))}
      <Spacer space={10} />
      ...
      {!isSaving && (
        <TouchableOpacity style={{ alignSelf: "center" }} onPress={showAddAttributeModal}>
        ...
        <ThemedView style={{ borderRadius: 25, paddingBottom: 20 }}>
          ...
          <View style={{ flexDirection: "row", alignItems: "center", alignSelf: "center", width: "80%" }}>
            <InputText
              ...
              onChangeText={(value: string) => setAttributeName(value)}
            />
            <Spacer hspace={5} />
            <View>

...
const FeeTypeDetails = () => {
  ...
  const { data: exceptionTypes, isFetching: isExceptionTypesLoading } = useGetExceptionQuery({
    feeTypeId: feeObj.clubFeeTypeId,
  });
  ...
  const editFeeType = () => {
    router.push({
      ...
      params: {
        clubFeeTypeId: fee.clubFeeTypeId,
        clubFeeAmount: fee.clubFeeAmount,
        clubFeeType: fee.clubFeeType,
        clubFeeTypeInterval: fee.clubFeeTypeInterval,
            ...
            <View style={{ width: "95%", alignSelf: "center" }}>
              ...
              {!isExceptionTypesLoading && (!exceptionTypes?.length || exceptionTypes?.length < 1) && (
                <ThemedText style={{ alignSelf: "center", width: "80%" }}>
                  No exceptions present for this fee type. You can add exceptions by pressing the + icon above. With
                  this feature you can configure special fees for some members for this fee type (eg. in case of leave)
                </ThemedText>
              )}

              <RoundedContainer>
                {!isExceptionTypesLoading &&
                  exceptionTypes?.map((et: any, idx: number) => (
        ...
        <View style={{ flex: 1 }}>
          ...
          {isLoading ? (
            ...
            <FlatList
              ...
              data={items}
              ListEmptyComponent={() => (
                <ThemedText style={{ alignSelf: "center", width: "85%" }}>
                  No collections present. To start collecting fee for a period, press the below button.
                </ThemedText>
              )}

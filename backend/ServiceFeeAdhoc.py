...
class FeeAdhocService():
    ...
    def post(self,conn, params):
        ...
        db.execute(conn, queries_fee.ADD_FEE_ADHOC, (clubAdhocFeeId, clubId, adhocFeeName, adhocFeeDesc, 1, email, email)) 

        for member in addedMembers:
            db.execute(conn, queries_fee.ADD_FEE_ADHOC_PAYMENT, (clubAdhocFeeId, member["membershipId"], member['clubAdocFeePaymentAmount'],email,email))

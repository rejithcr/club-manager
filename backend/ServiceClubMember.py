...
class ClubMemberService():
    ...
    def get(self, conn, params):
        ...
        if getClubMemberAttributeValues:
            member_attribute_values = db.fetch(conn, queries_member.GET_MEMBER_ATTRIBUTE_VALUES, (club_id, memberId, club_id))
    ...
    def post(self, conn, params):
        ...
        if addClubMemberAttribute:
            db.execute(conn, queries_member.ADD_MEMBER_ATTRIBUTE, (club_id, attributeName, 1 if required else 0, email, email))
            conn.commit()
        ...
        if not member_id: 
            ...
            db.execute(conn, queries_member.SAVE_MEMBER, (first_name, last_name, email, phone, None, 0, createdBy, createdBy))        
    ...
    def put(self, conn, params):
        ...
        if editClubMemberAttribute:
            db.execute(conn, queries_member.UPDATE_MEMBER_ATTRIBUTE, (attributeName, 1 if required else 0, email, clubMemberAttributeId))
            conn.commit()
        ...
        if saveClubMemberAttributeValues:
            ...
            for change in updatedCMAList:
                if change["attributeValue"] and change["attributeValue"].strip() != "":
                    db.execute(conn, queries_member.ADD_MEMBER_ATTRIBUTE_VALUE, (change["clubMemberAttributeId"],clubId,memberId,change["attributeValue"], email, email))
            conn.commit()
        ...
        db.execute(conn, queries_member.UPDATE_MEMBER, (first_name, last_name, email, phone, dateOfBirth, updatedBy, memberId))
        conn.commit()

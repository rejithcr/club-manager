GET_FEES = """
select cft.club_fee_type_id,club_id,club_fee_type,club_fee_type_interval,
...
GET_FEES_BY_ID = """
    select club_fee_type_id,club_id,club_fee_type,club_fee_type_interval,
            club_fee_is_active,club_fee_amount,club_fee_type_desc
    from club_fee_type
...
EDIT_FEE_TYPE = """
    ...
    set club_fee_type = %s, club_fee_type_interval = %s,
            club_fee_amount = %s, updated_by = %s
    where club_fee_type_id = %s
"""

DELETE_FEE_TYPE = """
    delete from club_fee_type_exception_member
    where club_fee_type_exception_id in (
        select club_fee_type_exception_id
        from club_fee_type_exception
        where club_fee_type_id = %s
    );

    delete from club_fee_type_exception
    where club_fee_type_id = %s;

    delete from club_fee_type
    where club_fee_type_id = %s;
"""

GET_FEE_TYPE_EXCEPTIONS = """
    select club_fee_type_exception_id, club_fee_type_id, club_fee_type_exception_reason,
...
GET_COLLECTIONS_OF_EXCEPTION = """
    ...
    from club_fee_type_exception cfte
        ...
        join club_fee_type_exception_member cftem on cftem.club_fee_type_exception_id = cfte.club_fee_type_exception_id
        join club_fee_payment cfp on cftem.club_fee_type_exception_member_id =cfp.club_fee_type_exception_member_id 
    where cfte.club_fee_type_exception_id = %s
"""

GET_FEE_TYPE_EXCEPTION = """
    select cfte.club_fee_type_exception_id, cfte.club_fee_type_id, cfte.club_fee_type_exception_reason,
    ...
    from club_fee_type_exception cfte
        left join club_fee_type_exception_member cftem on cfte.club_fee_type_exception_id = cftem.club_fee_type_exception_id
        left join membership ms on cftem.membership_id =ms.membership_id
        left join member m on m.member_id = ms.member_id
    where cfte.club_fee_type_exception_id = %s
    group by cfte.club_fee_type_exception_id, cfte.club_fee_type_id, cfte.club_fee_type_exception_reason, 
        cfte.club_fee_exception_amount
"""
...
ADD_FEE_TYPE_EXCEPTION = """
    insert into club_fee_type_exception(club_fee_type_exception_id, club_fee_type_id,
            club_fee_type_exception_reason,club_fee_exception_amount,club_fee_exception_is_active,
...
UPDATE_FEE_TYPE_EXCEPTION = """
    ...
    set club_fee_type_exception_reason = %s, club_fee_exception_amount = %s, updated_by = %s
    where club_fee_type_exception_id = %s
"""

ADD_FEE_TYPE_EXCEPTION_MEMBER = """
    insert into club_fee_type_exception_member(club_fee_type_exception_member_id,club_fee_type_exception_id,
        membership_id,start_date,created_by,updated_by)
...
UPDATE_FEE_TYPE_EXCEPTION_MEMBER_END_DATE = """
    ...
    set end_date = %s, updated_by = %s
    where club_fee_type_exception_member_id = %s
"""


GET_FEE_TYPE_EXCEPTION_MEMBERS = """
    select club_fee_type_exception_member_id, club_fee_type_exception_id, membership_id,
            start_date, end_date
...
GET_LATEST_COLLECTION_PERIOD = """
    ...
    where cft.club_fee_type_id = %s    
        and cfc.club_fee_type_date = (select max(club_fee_type_date) from club_fee_collection where club_fee_type_id = %s)    
...
ADD_FEE_TYPE_COLLECTION_START = """
    insert into club_fee_collection(club_fee_collection_id, club_fee_type_id,
            club_fee_type_period,club_fee_type_date, created_by, updated_by)
    values(%s,%s,%s,%s,%s,%s)
"""
...
DELETE_FEE_COLLECTION = """
    delete from club_fee_payment
    where club_fee_collection_id = %s;

    delete from club_fee_collection
    where club_fee_collection_id = %s;
"""
...
GET_FEE_ADHOC_COLLECTION_BY_ID = """
    select caf.club_id, caf.club_adhoc_fee_name,caf.club_adhoc_fee_desc,
    caf.club_adhoc_fee_is_active, sum(cafp.club_adhoc_fee_payment_amount) club_adhoc_fee_payment_amount, 
    json_agg(json_build_object(

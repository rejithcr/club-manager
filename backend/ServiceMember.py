...
class MemberService():
    ...
    def post(self, conn, params):
        ...
        db.execute(conn, queries_member.SAVE_MEMBER, (first_name, last_name, email, phone, photo, dateOfBirth, isRegistered, createdBy, createdBy))
        conn.commit()
        member = db.fetch_one(conn, queries_member.GET_MEMBER_BY_EMAIL, (email,))
    ...
    def put(self, conn, params):
        ...
        if verify:
            db.execute(conn, queries_member.VERIFY_MEMBER, (first_name, last_name, email, phone, photo, dateOfBirth, isRegistered, updatedBy, memberId))
            conn.commit()
        else:

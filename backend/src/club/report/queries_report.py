GET_CLUB_MEMBER_ATTRIBUTES_VALUES = """
    ...
    coalesce(json_agg(distinct jsonb_build_object('attribute', cma."attribute", 'attribute_value', cmav.attribute_value)) 
    filter (where cma.club_member_attribute_id = ANY(%s)), '[]'::json) attributes
    from member m 

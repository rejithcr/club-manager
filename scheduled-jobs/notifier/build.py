import os
import shutil
import zipfile

src = "src/"
dir_dst = "build/"

src_files = os.listdir(src)

#cleanup
if os.path.exists(dir_dst) and os.path.isdir(dir_dst):
    shutil.rmtree(dir_dst)
os.mkdir(dir_dst)
try:
    os.remove("job.zip")
except OSError:
    pass

# Install Linux-compatible wheels explicitly.
# --platform + --only-binary forces pip to download manylinux wheels
# (Amazon Linux 2 compatible) instead of Windows .pyd binaries,
# even when this script runs on Windows.
os.system(
    f"pip install "
    f"--platform manylinux2014_x86_64 "
    f"--implementation cp "
    f"--python-version 3.13 "
    f"--only-binary=:all: "
    f"--target {dir_dst} "
    f"-r requirements.txt"
)

shutil.copytree(src, dir_dst+src, dirs_exist_ok=True)

def zipfolder(filename, target_dir):
    zip_obj = zipfile.ZipFile(filename + '.zip', 'w', zipfile.ZIP_DEFLATED)
    root_len = len(target_dir)
    for base, dirs, files in os.walk(target_dir):
        for file in files:
            fn = os.path.join(base, file)
            zip_obj.write(fn, fn[root_len:])

zipfolder('job', dir_dst) #insert your variables here
